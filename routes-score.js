...
// Sources: your own endpoints + FinMind fallback (no Yahoo, no keys).
...
async function finmindPriceHistory(code, days=60){
  try{
    ...
    const { data } = await axios.get(url, { params: { dataset:'TaiwanStockPrice', stock_id: code, start_date: s, end_date: `${yyyy}-${mm}-${dd}` }, timeout: 12000, headers:{'User-Agent':UA} });
    return data?.data || [];
  }catch{ return []; }
}
...
async function finmindInstFlows(code, days=5){
  try{
    ...
    const s = `${start.getFullYear()}-${String(start.getMonth()+1).padStart(2,'0')}-${String(start.getDate()).padStart(2,'0')}`;
    const url = 'https://api.finmindtrade.com/api/v4/data';
    const { data } = await axios.get(url, { params: { dataset:'TaiwanStockInstitutionalInvestorsBuySell', stock_id: code, start_date: s, end_date: `${yyyy}-${mm}-${dd}` }, timeout: 12000, headers:{'User-Agent':UA} });
...
function atr14(rows){
  ...
  const cs = rows.map(r=>Number(r.close||r.Close));
  const hs = rows.map(r=>Number(r.max||r.high||r.High));
  const ls = rows.map(r=>Number(r.min||r.low ||r.Low));
  const trs = [];
...
const highN = (rows,n=20)=> rows.slice(-n).reduce((m,r)=> Math.max(m, Number(r.max||r.high||r.High)||-Infinity), -Infinity);
const lowN  = (rows,n=20)=> rows.slice(-n).reduce((m,r)=> Math.min(m, Number(r.min||r.low ||r.Low )|| Infinity),  Infinity);
  ...
  app.get('/score/confluence', async (req, res)=>{
    ...
    try{
      ...
      for (const code of codes){
        ...
        const hist = await finmindPriceHistory(code, 60);
        if (!hist.length){ out.push({ code, name: names[code] || sym.byCode?.[code] || '', score:null, reason:'no_history' }); continue; }

        const closes = hist.map(r=>Number(r.close||r.Close)).filter(n=>isFinite(n));
        const vols   = hist.map(r=>Number(r.Trading_Volume||r.volume)).filter(n=>isFinite(n));
        ...
        out.push({
          ...
          refClose: isFinite(close) ? close : null,

...
const LISTS_PATH    = process.env.LISTS_PATH || './data/lists.json';   // 你原本 .env.example 就有
const SYMBOLS_PATH  = process.env.SYMBOLS_PATH || './symbols.json';    // 新增但有預設
const CRON_KEY      = process.env.CRON_KEY || '';                      // 沿用既有 key
...
module.exports = function mountLists(app){
  ...
  // 寫清單（只給你/機器人；Header: x-webhook-key = CRON_KEY）
    ...
    if(action==='add'){
      ...
      if((!c || !/^\d{4}$/.test(c)) && name){
        try{
          ...
          if(sy?.byName?.[NN]) cand.push(...sy.byName[NN]);                   // 完整命中
          for(const [k,v] of Object.entries(sy?.byCode||{})){                  // 模糊包含
            if(normName(v).includes(NN)) cand.push(k);
          }
  ...
  // 名稱/代號查詢（給 Bot；先跑 /symbols/refresh 建索引）
  app.get('/symbols/lookup', async (req,res)=>{
    ...
    if(/^\d{4}$/.test(q) && sy.byCode[q]) out.set(q, sy.byCode[q]);           // 代號命中
    (sy.byName?.[n]||[]).forEach(c=> out.set(c, sy.byCode[c]||''));           // 全名命中
    for(const [c, nm] of Object.entries(sy.byCode)){                          // 模糊包含
      if(normName(nm).includes(n)) out.set(c, nm);
    }
    res.json({ matches: Array.from(out.entries()).slice(0,10).map(([code,name])=>({code,name})) });
  });

  // 建立全市場名錄（只允許你；Header x-webhook-key: CRON_KEY）
  app.post('/symbols/refresh', async (req,res)=>{
    if(!CRON_KEY || req.header('x-webhook-key')!==CRON_KEY) return res.status(403).json({error:'forbidden'});
    try{
      // 來源：FinMind TaiwanStockInfo（免金鑰）；之後要換官方 CSV 也可
